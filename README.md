Sallinger Home Videos
==============================

Project to display our home videos hosted on google photos
...
```bash
conda create python=3.8 -n sallinger_home_videos
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate sallinger_home_videos
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///sallinger_home_videos` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE sallinger_home_videos WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE sallinger_home_videos TO ;
```
...
ERROR:  database "sallinger_home_videos" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists sallinger_home_videos
...
```bash
dropdb --if-exists sallinger_home_videos
